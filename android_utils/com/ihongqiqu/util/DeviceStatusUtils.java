...
public class DeviceStatusUtils
{
    ...
    public static boolean setScreenBrightnessMode(Context context, boolean auto)
    {
        ...
        {
            result = Settings.System.putInt(context.getContentResolver(), Settings.System.SCREEN_BRIGHTNESS_MODE, auto
    ...
    public static boolean setScreenBrightness(Context context, int screenBrightness)
    {
        ...
        boolean result = Settings.System.putInt(context.getContentResolver(), Settings.System.SCREEN_BRIGHTNESS,
                brightness);
        return result;
    }
    ...
    public static boolean setAirplaneMode(Context context, boolean enable)
    {
        ...
        if (isAirplaneModeOpen(context) != enable)
        {
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN_MR1)
            {
                result = Settings.System.putInt(context.getContentResolver(), Settings.System.AIRPLANE_MODE_ON,
                        enable ? 1 : 0);
            }
            else
            {
                result = Settings.Global.putInt(context.getContentResolver(), Settings.Global.AIRPLANE_MODE_ON,
                        enable ? 1 : 0);
            }
            // 发送飞行模式已经改变广播
